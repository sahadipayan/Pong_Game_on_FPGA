...
ENTITY vga_controller IS
  GENERIC(
    ...
    h_bp     : INTEGER := 216;    --horiztonal back porch width in pixels
    h_pixels : INTEGER := 1280;   --horiztonal display width in pixels
    h_fp     : INTEGER := 80;    --horiztonal front porch width in pixels
    h_pol    : STD_LOGIC := '0';  --horizontal sync pulse polarity (1 = positive, 0 = negative)
    v_pulse  : INTEGER := 3;      --vertical sync pulse width in rows
    v_bp     : INTEGER := 30;     --vertical back porch width in rows
    v_pixels : INTEGER := 960;   --vertical display width in rows
